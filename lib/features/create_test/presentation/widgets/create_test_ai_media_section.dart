  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CreateTestCubit, CreateTestState>(
      ...
      builder: (context, state) {
        ...
        if (!state.isAiMode || (!hasLocalMedia && !hasExistingMedia)) {
          return const SizedBox.shrink();
        }

        final isImages = state.isAiImages;
...
class _ExistingImagePreviewTile extends StatelessWidget {
  ...
  const _ExistingImagePreviewTile({
    required this.media,
  });
