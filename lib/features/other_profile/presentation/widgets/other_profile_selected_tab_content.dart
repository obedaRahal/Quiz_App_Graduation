...
class OtherProfileSelectedTabContent extends StatelessWidget {
  final OtherProfileTab selectedTab;
  final OtherProfileOverviewDataEntity overviewDataEntity;
  final int userId;

  const OtherProfileSelectedTabContent({
    super.key,
    ...
    return BlocBuilder<OtherProfileCubit, OtherProfileState>(
      ...
      builder: (context, state) {
        ...
        return OtherProfileContentTab(
          ...
          onSaveTap: (contentId) {
            debugPrint("contentId is $contentId");
            context.read<OtherProfileCubit>().toggleContentBookmark(
              contentId: contentId,
            );
          },
          onLikeTap: (contentId) {},
