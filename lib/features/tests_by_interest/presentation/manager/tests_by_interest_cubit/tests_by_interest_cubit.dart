...
class TestsByInterestCubit extends SafeCubit<TestsByInterestState> {
  ...
  void searchTests({required int interestId, required String query}) {
  ...
  Future<void> loadMoreSearchResults({required int interestId}) async {
    if (!state.isSearchMode ||
    ...
    try {
      ...
      emit(
        state.copyWith(
          ...
          searchResults: [...state.searchResults, ...response.tests],
          searchCurrentPage: response.meta.currentPage,
          searchLastPage: response.meta.lastPage,
          searchHasMorePages: response.meta.hasMorePages,
          clearSearchLoadMoreErrorMessage: true,
